...
/*package*/ class LoadedDocuments implements Constants {
    ...
    public LoadedDocuments(KeySpaces keySpaces, String[] documentLoaderOptions,
    ...
    private LoadedDocument preload(URL url) 
        ...
        if (doc == null) {
            ...
            if (doc == null) {
                ...
                Document loaded = loadDocument(url, validate, console);
    ...
    private Document loadDocument(URL url, boolean validate, 
        ...
        try {
            loader = 
                DocumentLoaderFactories.createLoader(url, documentLoaderOptions,
                                                     console);
        } catch (Exception e) {
            throw new IOException(Msg.msg("cannotCreateDocumentLoader", 
                                          URLUtil.toLabel(url),
        ...
        Document doc;
        if (loader == null) {
            doc = LoadDocument.load(url, validate, console);
        } else {
            try {
                doc = loader.load(url, validate, console);
            } catch (IOException e) {
                throw e;
            } catch (Exception e) {
                throw new IOException(Msg.msg("cannotLoad", 
                                              URLUtil.toLabel(url),
                                              ThrowableUtil.reason(e)));
            }
        }
    ...
    private final void resolveObjectURLs(Element element, URL baseURL) {
        ...
        for (Element param : params) {
            ...
            if ("ref".equals(valuetype) ||
                (("source".equals(name) || "track".equals(name) || 
                  "poster".equals(name)) &&
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (; i < args.length; ++i) {
            ...
            if ("-k".equals(arg)) {
                ...
                KeyLoader keyLoader = new KeyLoader(null, null);
                ...
                LoadedDocuments loadedTopics = 
                    new LoadedDocuments(null, null, null);
        ...
        LoadedDocuments loadedDocs = new LoadedDocuments(keySpaces, null, null);
        ...
        case BOOKMAP:
            if (keySpaces != null) {
                if (!(new MapSimplifier(keySpaces, null,
