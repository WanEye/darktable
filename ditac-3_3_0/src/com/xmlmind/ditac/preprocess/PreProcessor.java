...
public class PreProcessor implements Constants {
    ...
    protected String[] documentLoaderOptions;
    ...
    /**
     * Specifies the options used to create specialized {@link DocumentLoader}s 
     * (for example MDITA loaders).
     * <p>An option name must start with prefix 
     * <tt>load.<i>loader_factory_name</i>.</tt>.
     * <p>Initial value is an empty list.
     *
     * @see #getDocumentLoaderOptions
     */
    public void setDocumentLoaderOptions(String[] options) {
        if (options == null) {
            options = StringUtil.EMPTY_LIST;
        }
        documentLoaderOptions = options;
    }

    /**
     * Returns the options used to create specialized {@link DocumentLoader}s
     * (for example MDITA loaders).
     *
     * @see #setDocumentLoaderOptions
     */
    public String[] getDocumentLoaderOptions() {
    ...
    protected boolean process1(URL[] inFiles, File outDir, String baseName) 
        ...
        mainMap = null;
        loadedDocs = new LoadedDocuments(/*keySpaces*/ null, 
                                         documentLoaderOptions, console);
        ...
        if (mainMap == null) {
            ...
            KeyLoader keyLoader = new KeyLoader(documentLoaderOptions, console);
            ...
            if (!(new MapSimplifier(keySpaces, documentLoaderOptions,
    ...
    protected boolean transcludeTopics1(LoadedDocument[] loadedTopicDocs) 
        ...
        return (new ConrefIncluder(keySpaces, documentLoaderOptions,
    ...
    protected boolean transcludeTopics2(LoadedDocument[] loadedTopicDocs) 
        ...
        (new ConrefPusher(documentLoaderOptions, 
                          console)).process(loadedTopicDocs);
        return true;
    }
    ...
    protected void addRelatedLinks(Element mapElement) {
        ...
        (new LinkGenerator(console)).processMap(mapElement, mainMap.url, 
                                                loadedDocs);
    }
    ...
    protected static boolean addLinkText(Element link, Element target) {
        ...
        if (linktext != null) {
            String text = DITAUtil.getTextContent(linktext);
        ...
        if (desc != null) {
            String text = DITAUtil.getTextContent(desc);
            if (text == null || text.length() == 0) {
                // Remove empty desc.
            ...
            if (shortForm == null) {
                ...
                if (longForm != null &&
                    (title = XMLText.collapseWhiteSpace(
                        DITAUtil.getTextContent(longForm))).length() > 0) {
    ...
    protected boolean addXrefText(Element xref, Element target) {
        ...
        if (DITAUtil.hasClass(target, "topic/fn") &&
            ...
            if (!isLwDITA(xref)) {
                console.warning(xref,
                                Msg.msg("missingAttribute2", "type", "fn"));
            }
            // LwDITA XDITA has not xref/@type, so no warning.
    ...
    protected static boolean isLwDITA(Element element) {
        String domains = DOMUtil.lookupAttribute(element, null, "domains");
        if (domains != null &&
            (domains = XMLText.collapseWhiteSpace(domains)).length() > 0) {
            return domains.indexOf(" xdita-c)") > 0;
        }
    ...
    protected boolean processObject(Element element, File outDir) {
        ...
        for (int i = 0; i < paramCount; ++i) {
            ...
            if ("source".equals(name)) {
                ...
                if (!processResource(param, "value", null, false, outDir)) {
                    return false;
                }
    ...
    protected void numberListingLines(Chunk[] chunks) {
        console.verbose(Msg.msg("numberListingLines"));
        ...
        for (int i = 0; i < chunks.length; ++i) {
            ...
            for (int j = 0; j < entries.length; ++j) {
                ...
                if (element != null) { // That is, a topic.
                    if (!processed.containsKey(element)) {
                        ListingProcessor.processTopic(element, console);
        ...
        while (child != null) {
            if (child.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (DITAUtil.hasClass(childElement, "topic/title")) {
                    String title = DITAUtil.getTextContent(childElement);
                    if (title != null && 
                        (title = 
