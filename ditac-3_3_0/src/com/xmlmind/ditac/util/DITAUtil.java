...
public final class DITAUtil implements Constants {
    ...
    /**
     * Same {@link Node#getTextContent} which filters out comments and PIs 
     * but <em>not</em> &lt;draft-comment&gt; and &lt;required-cleanup&gt;.
     */
    ...
    private static void getTextContent(Element element, StringBuilder buffer) {
        Node child = element.getFirstChild();
        while (child != null) {
            ...
            case Node.ELEMENT_NODE:
                {
                    ...
                    if (!DITAUtil.hasClass(childElement, 
                        ...
                        getTextContent(childElement, buffer);
                    }
                }
                break;
            }
    ...
    public static URL doGetLocalTopicURL(Element topicref) 
        ...
        String format = DITAUtil.getFormat(topicref, href, scope);
        if (format == null) {
    ...
    public static String getFormat(Element topicref, String href,
        ...
        return getFormat(format, href, scope);
    }

    public static String getFormat(String format, String href, 
                                   String scope) {
        if (format != null) {
            int pos = format.indexOf(';'); // Example: "text; charset=UTF-8"
            if (pos >= 0) {
        ...
        if (format == null && href != null) {
            format = formatFromExtension(href);
        }

        if (format != null && 
            (scope == null || "local".equals(scope))) {
            if ("mdita".equals(format) ||
                "markdown".equals(format) ||
                "hdita".equals(format) ||
                "html".equals(format)) {
                format = "dita";
            }
    ...
    private static String formatFromExtension(String href) {
        String format = null;
