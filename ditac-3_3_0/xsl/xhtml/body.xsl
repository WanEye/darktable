...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <xsl:call-template name="processObjectDesc">
              <xsl:with-param name="objectId" select="normalize-space(@id)"/>
            </xsl:call-template>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
  ...
  <xsl:template name="processAudioVideoParams">
    ...
    <xsl:for-each select="$params">
      <xsl:variable name="paramName" select="normalize-space(@name)"/>
      <xsl:variable name="paramValue" select="normalize-space(@value)"/>
    ...
    <source src="{normalize-space(@data)}">
  ...
  <xsl:template name="generateObjectFallback">
    <xsl:param name="container" select="'div'"/>
    <xsl:param name="objectId" select="''"/>
    ...
    <xsl:if test="$download ne ''">
      <xsl:element name="{$container}">
        <xsl:attribute name="class" select="'object-auto-fallback'"/>
